...
/**
 * Phase 0 admin gate — shared secret in localStorage. Replaced by Clerk
 * role gating in Phase 1.
 */
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Verify by hitting GET /api/uploads with the header
    fetch('/api/admin/check', { headers: { 'x-admin-secret': saved } })
      .then((r) => {
        if (r.status !== 401) setUnlocked(true);
        else {
          window.localStorage.removeItem('stayops_admin');
          setUnlocked(false);
        }
      })
