...
import { Filter, Search, Wrench, AlertTriangle, List, LayoutGrid } from 'lucide-react';
...
export default function EmmaRoomsPage() {
  ...
  // Floor groups for the grid view (built from the filtered set).
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {/* View toggle */}
        <div className="ml-auto inline-flex rounded-lg p-0.5" style={{ background: '#f7f7f7', border: '1px solid #dddddd' }}>
          <button
            onClick={() => setView('list')}
            className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-semibold transition-colors"
            style={{ background: view === 'list' ? '#ffffff' : 'transparent', color: view === 'list' ? '#222' : '#6a6a6a' }}
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-semibold transition-colors"
            style={{ background: view === 'grid' ? '#ffffff' : 'transparent', color: view === 'grid' ? '#222' : '#6a6a6a' }}
          >
      ...
      {/* Grid view — floor-grouped room tiles (front-desk style) */}
      {view === 'grid' ? (
        <div className="rounded-2xl overflow-hidden" style={{ background: '#ffffff', border: '1px solid #dddddd' }}>
          {/* Legend */}
          <div className="px-5 py-2.5 flex items-center gap-5 flex-wrap" style={{ background: '#fafafa', borderBottom: '1px solid #dddddd' }}>
            ...
            <span className="flex items-center gap-1.5 text-xs" style={{ color: '#6a6a6a' }}>
              <span className="w-3 h-3 rounded-full flex items-center justify-center" style={{ background: '#ff385c' }}>
                <Wrench className="w-2 h-2" style={{ color: '#fff' }} />
              </span>
              Open ticket
            </span>
          </div>
            ...
            {floorGroups.length === 0 ? (
              ...
              <div key={floor} className="mb-5 last:mb-0">
                ...
                <div className="flex flex-wrap gap-2">
                  {frooms.sort((a, b) => a.number.localeCompare(b.number)).map((room) => {
                    const cfg = ROOM_TILE[room.status] ?? ROOM_TILE.occupied;
                    const tk = ticketsByRoom.get(room.number) ?? 0;
                    return (
                      <div
                        key={room.id}
                        ...
                        title={`Room ${room.number} · ${cfg.label}${tk > 0 ? ` · ${tk} ticket${tk === 1 ? '' : 's'}` : ''}`}
                      >
