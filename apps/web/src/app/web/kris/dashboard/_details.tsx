...
/**
 * Per-KPI detail panels for Kris's dashboard drawer. Standalone — its own small
...
export interface ScopedData {
  hotels: Hotel[];
  revenueRows: ApiRevenueSummary[];
  labourRows: ApiLabourMetrics[];
  dailyRows: ApiDailyMetrics[];
}
      ...
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr style={{ borderBottom: '1px solid #dddddd', background: '#f7f7f7' }}>
            {columns.map((c, i) => (
              <th key={c} className="text-[10px] font-bold uppercase tracking-wide py-3 px-4 whitespace-nowrap" style={{ color: '#6a6a6a', textAlign: i === 0 ? 'left' : 'right' }}>{c}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {rows.map((r, i) => (
            <tr key={i} style={i < rows.length - 1 ? { borderBottom: '1px solid #f0f0f0' } : undefined}>
              {r.map((cell, j) => (
                <td key={j} className="py-2.5 px-4 text-sm" style={{ textAlign: j === 0 ? 'left' : 'right', color: j === 0 ? '#222' : '#3f3f3f' }}>{cell}</td>
              ))}
            </tr>
          ))}
        </tbody>
...
export function OccupancyDetail({ hotels, revenueRows, dailyRows }: ScopedData) {
  const rows = hotels
    .map((h) => ({ h, rev: revenueRows.find((r) => r.hotelId === h.id), dm: dailyRows.find((d) => d.hotelId === h.id) }))
    .sort((a, b) => (a.rev?.occupancyPct ?? 0) - (b.rev?.occupancyPct ?? 0));
  return (
    <div className="flex flex-col gap-5">
      <p className="text-sm" style={{ color: '#3f3f3f', lineHeight: 1.55 }}>Occupancy per hotel, weakest to strongest. Anything under 70% is worth a look.</p>
      <Table
        columns={['Hotel', 'Sold', 'Rooms', 'Occ %']}
        rows={rows.map(({ h, rev, dm }) => [
          <span key="n"><span className="font-medium" style={{ color: '#222' }}>{h.shortName}</span><span className="text-xs ml-2" style={{ color: '#929292' }}>{h.city}, {h.state}</span></span>,
          dm?.roomsSold ?? '—',
          h.rooms,
          <span key="o" style={{ color: (rev?.occupancyPct ?? 0) < 70 ? '#b91c1c' : (rev?.occupancyPct ?? 0) < 80 ? '#b45309' : '#15803d', fontWeight: 600 }}>{formatPct(rev?.occupancyPct ?? 0, 1)}</span>,
        ])}
      />
    </div>
  );
...
export function RevenueDetail({ hotels, revenueRows }: ScopedData) {
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="rounded-2xl p-5" style={{ background: '#fff1f3', border: '1px solid rgba(255,56,92,0.3)' }}>
        <p className="text-[10px] font-bold uppercase tracking-wide" style={{ color: '#ff385c' }}>Total revenue — in scope</p>
        <p className="text-3xl font-bold tabular-nums mt-1" style={{ color: '#222' }}>{formatCurrency(total, true)}</p>
        <p className="text-xs mt-1" style={{ color: '#6a6a6a' }}>{hotels.length > 1 ? `Across ${hotels.length} hotels` : 'This property'}</p>
      </div>
      <div>
        <p className="text-xs font-bold uppercase tracking-wide mb-3" style={{ color: '#6a6a6a' }}>Revenue mix by source · tap to expand</p>
        <MdRevenueMix hotelIds={hotels.map((h) => h.id)} />
      </div>
    </div>
  );
}
      ...
      {rows.map(({ h, oooRooms }) => (
        <div key={h.id} className="rounded-2xl p-4" style={{ border: '1px solid #dddddd', background: '#fff' }}>
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs font-semibold" style={{ color: oooRooms.length > 0 ? '#b91c1c' : '#15803d' }}>{oooRooms.length} OOO</span>
          </div>
          {oooRooms.length === 0 ? (
...
export function StaleDirtyDetail({ hotels }: ScopedData) {
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="rounded-2xl p-4" style={{ background: '#fff1f3', border: '1px solid rgba(255,56,92,0.3)' }}>
        <p className="text-sm font-semibold" style={{ color: '#ff385c' }}>What is &quot;Stale Dirty&quot;?</p>
        <p className="text-sm mt-1.5" style={{ color: '#3f3f3f', lineHeight: 1.55 }}>Rooms sitting in <strong>Dirty</strong> status for 2+ days with <strong>no open maintenance ticket</strong> — candidates for an OOO flag that hasn&apos;t been filed.</p>
      </div>
      {rows.length === 0 ? (
        <div className="rounded-2xl p-8 text-center" style={{ border: '1px solid #dddddd', background: '#fff' }}>
          ...
          <p className="text-xs mt-1" style={{ color: '#929292' }}>No stale-dirty rooms across the scope.</p>
        </div>
      ) : rows.map(({ h, stale }) => (
        <div key={h.id} className="rounded-2xl p-4" style={{ border: '1px solid #dddddd', background: '#fff' }}>
          <div className="flex items-center justify-between mb-2">
            <p className="text-sm font-semibold" style={{ color: '#222' }}>{h.shortName}</p>
            <span className="text-xs font-semibold" style={{ color: '#b45309' }}>{stale.length} stale dirty</span>
          </div>
  ...
  return (
    <Table
      ...
      rows={rows.map(({ h, lab }) => [
        ...
        <span key="v" style={{ color: (lab?.variance ?? 0) > 20 ? '#b91c1c' : (lab?.variance ?? 0) > 0 ? '#b45309' : '#15803d', fontWeight: 600 }}>{formatVariance(lab?.variance ?? 0)}</span>,
        <span key="o" style={{ color: (lab?.overtimeHours ?? 0) > 15 ? '#b91c1c' : '#3f3f3f' }}>{lab?.overtimeHours ?? 0} hrs</span>,
      ])}
    />
  );
}
  ...
  return (
    <Table
      ...
      rows={rows.map(({ h, rev, lab, pct }) => [
        ...
        <span key="p" style={{ color: pct > 28 ? '#b91c1c' : pct > 24 ? '#b45309' : '#15803d', fontWeight: 600 }}>{formatPct(pct, 1)}</span>,
      ])}
    />
  );
}
  ...
  return (
    <Table
      ...
      rows={rows.map(({ h, dm }) => {
        ...
        const tier = mdCsatTier(rating);
        return [
          <span key={h.id} className="font-medium" style={{ color: '#222' }}>{h.shortName}</span>,
          <span key="r" style={{ color: tier.alert ? '#b91c1c' : '#222', fontWeight: 600 }}>{rating.toFixed(1)} / 5.0</span>,
          <span key="t" style={{ color: tier.alert ? '#b91c1c' : '#3f3f3f' }}>{tier.label}</span>,
        ];
      })}
    />
  );
