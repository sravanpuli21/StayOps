...
export default function StoryDrivenHybrid() {
  return (
    <main>
      <PreviewBadge />
      <Hero />
...
function PreviewBadge() {
  return (
    <div className="text-center py-2 px-4 text-xs" style={{ background: '#0a0a0a', color: '#fff' }}>
      ...
      Story-driven · <strong className="font-semibold">Hybrid (7 sections)</strong>
    </div>
  );
}
  ...
  return (
    <section className="relative" style={{ height: 'min(78vh, 760px)' }}>
      ...
      <div className="relative z-10 h-full mx-auto max-w-6xl px-5 sm:px-8 flex flex-col justify-end pb-16 sm:pb-20 text-white">
        ...
        <h1 style={{ fontSize: 'clamp(2.5rem, 6vw, 5rem)', lineHeight: 1.02, letterSpacing: '-0.03em', maxWidth: '18ch', fontWeight: 600 }} className="mt-5">
        ...
        <p className="mt-5 text-base sm:text-lg lg:text-xl" style={{ color: 'rgba(255,255,255,0.85)', maxWidth: '44ch', lineHeight: 1.5 }}>
          Rooms turn. Repairs queue. Audits age. Handovers drift. By the time the owner asks, the answer is gone. StayOps keeps it.
        </p>
        <div className="mt-8 flex items-center gap-3 flex-wrap">
          ...
          <Link href="#modules" className="inline-flex items-center gap-2 rounded-full px-6 py-3 text-sm font-semibold text-white" style={{ border: '1px solid rgba(255,255,255,0.5)' }}>
            See the product
          </Link>
        </div>
      </div>
...
function QuoteStrip() {
  return (
    <section className="mx-auto max-w-4xl px-5 sm:px-8 py-20 sm:py-28 text-center">
      ...
      >
        &ldquo;I didn&apos;t need another dashboard. I needed the hotel to remember what happened yesterday.&rdquo;
      </p>
...
function ThreeScenes() {
  const scenes = [
    { img: IMG.hallway,   eyebrow: '11:08 PM', headline: 'A housekeeper finds a broken AC.',     body: 'Room 214. She tells the front desk. The desk is checking in a guest. The note gets written on a sticky.' },
    { img: IMG.reception, eyebrow: '7:42 AM',  headline: 'Morning handover runs thin.',         body: 'Night Audit left three open items. The AM shift catches one. Two drift into the day, unknown to everyone but the sticky.' },
    { img: IMG.housekeep, eyebrow: '1:30 PM',  headline: 'The GM gets the call.',                body: 'Guest refused Room 214. Another guest moved. Everyone agrees something broke on Tuesday. Nobody can say exactly when.' },
  ];
  return (
    <section style={{ background: '#0a0a0a', color: '#fff' }}>
      <div className="mx-auto max-w-6xl px-5 sm:px-8 py-20 sm:py-28">
        <p className="text-xs font-semibold uppercase tracking-[0.14em]" style={{ color: '#ff385c' }}>What StayOps replaces</p>
        <h2 className="mt-3" style={{ fontSize: 'clamp(1.75rem, 3.5vw, 2.75rem)', lineHeight: 1.12, letterSpacing: '-0.02em', fontWeight: 600, maxWidth: '24ch' }}>
          A hotel running on memory, phone calls, and sticky notes.
        </h2>
        <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-5">
          {scenes.map((s) => (
            <div key={s.eyebrow} className="rounded-2xl overflow-hidden" style={{ background: '#141414', border: '1px solid #222' }}>
              <div className="aspect-[4/3] overflow-hidden">
                <img src={s.img} alt="" className="w-full h-full object-cover" />
              </div>
              <div className="p-5">
                <p className="text-[11px] font-semibold uppercase tracking-[0.14em]" style={{ color: '#ff385c' }}>{s.eyebrow}</p>
                <p className="mt-2 text-lg font-semibold" style={{ lineHeight: 1.3 }}>{s.headline}</p>
                <p className="mt-2 text-sm" style={{ color: '#a8a8a8', lineHeight: 1.55 }}>{s.body}</p>
              </div>
            </div>
          ))}
        </div>
...
/* ---------------- Product modules (new in hybrid) ---------------- */
function ProductModules() {
  return (
    <section id="modules" className="mx-auto max-w-6xl px-5 sm:px-8 py-20 sm:py-28">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-start">
        <div className="lg:col-span-5 lg:sticky lg:top-24">
          ...
          <p className="mt-5 text-base sm:text-lg" style={{ color: '#3f3f3f', lineHeight: 1.65, maxWidth: '42ch' }}>
            Every room, every ticket, every audit, every handover — tracked, timestamped, and
            visible to the owner without a phone call.
          </p>
        </div>
...
/* ---------------- Before / After (new in hybrid) ---------------- */
function BeforeAfter() {
  return (
    <section className="relative overflow-hidden" style={{ background: '#f5f2ec' }}>
      <img src={IMG.room2} alt="" className="absolute inset-0 w-full h-full object-cover opacity-[0.12]" style={{ mixBlendMode: 'multiply' }} />
        ...
        <div className="mt-10 rounded-2xl overflow-hidden" style={{ background: '#fff', border: '1px solid #d4d0c5' }}>
          ...
          {BEFORE_AFTER.map(([before, after], i) => (
            <div
              ...
              style={i < BEFORE_AFTER.length - 1 ? { borderBottom: '1px solid #ede8dc' } : undefined}
            >
...
function OperatorsNote() {
  return (
    <section className="relative overflow-hidden" style={{ background: '#f5f2ec' }}>
      <img src={IMG.night} alt="" className="absolute inset-0 w-full h-full object-cover opacity-[0.18]" style={{ mixBlendMode: 'multiply' }} />
      <div className="relative mx-auto max-w-4xl px-5 sm:px-8 py-20 sm:py-28">
        <p className="text-xs font-semibold uppercase tracking-[0.14em]" style={{ color: '#b64a3f' }}>A note from the operators</p>
