...
interface SchedulingBoardProps {
  /** Base path for employee profile links, e.g. "/web/rishab/employee". */
  employeeHrefBase?: string;
}

export function SchedulingBoard({ employeeHrefBase = '/web/rishab/employee' }: SchedulingBoardProps) {
    ...
    <div className="flex flex-col gap-6">
      ...
      {activeWeekSched && activeTeam === 'Front Desk' && (
        <FrontDeskCalendar
          ...
          employeeHrefBase={employeeHrefBase}
        />
      )}
                ...
                return (
                  <tr key={emp.id} style={{ borderBottom: i < teamEmployees.length - 1 ? '1px solid #f0f0f0' : 'none' }}>
                    <td className="py-2 px-4">
                      <Link href={`${employeeHrefBase}/${emp.id}`} className="flex items-center gap-2 hover:underline">
...
function FrontDeskCalendar({
  weekSchedule, employees, dailyFd, gaps, onGapClick, employeeHrefBase,
}: {
